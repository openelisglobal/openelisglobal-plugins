...
public class HttpHl7TemplateAnalyzerMenu extends MenuPlugin {
    ...
    protected void insertMenu() {
		...
		menu.setActionURL("/AnalyzerResults?type=HttpTemplateAnalyzer");
        // The key used for the name of the analyzer on the menu. Should not already
        // exist in MessageResource.properties.
		menu.setDisplayKey("banner.menu.results.HttpTemplateanalyzer");
        menu.setOpenInNewWindow(false);

        service.addMenu(menu);
        // Analyzer name in English
		service.insertLanguageKeyValue("banner.menu.results.HttpTemplateAnalyzer", "HttpTemplateAnalyzer",
